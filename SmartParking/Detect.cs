    ...
    class Detect
    {
                ...
                for (double value = min; value <= max; value += 0.1)
                {
                    ...
                    using (MemStorage storage = new MemStorage())
                    {
                        ...
                        while (contours != null)
                        {
                            ...
                            CvInvoke.cvDrawContours(color2, contours, new MCvScalar(255, 255, 0), new MCvScalar(0), -1, 1, Emgu.CV.CvEnum.LINE_TYPE.EIGHT_CONNECTED, new Point(0, 0)); //Emgu.CV.CvEnum.LINE_TYPE.EIGHT_CONNECTED
                            ...
                            {
                                c++;
                                CvInvoke.cvDrawContours(color2, contours, new MCvScalar(0, 255, 255), new MCvScalar(255), -1, 3, Emgu.CV.CvEnum.LINE_TYPE.EIGHT_CONNECTED, new Point(0, 0)); //Emgu.CV.CvEnum.LINE_TYPE.EIGHT_CONNECTED
        ...
        private double cout_avg(Image<Gray, byte> src)
        {
            ...
            CvInvoke.cvAdaptiveThreshold(src, grayImage, 255,
                Emgu.CV.CvEnum.ADAPTIVE_THRESHOLD_TYPE.CV_ADAPTIVE_THRESH_MEAN_C, Emgu.CV.CvEnum.THRESH.CV_THRESH_BINARY, 21, 2); //CvInvoke.cvAdaptiveThreshold(src, grayImage, 255,Emgu.CV.CvEnum.ADAPTIVE.CV_ADAPTIVE_THRESH_MEAN_C, Emgu.CV.CvEnum.THRESH.CV_THRESH_BINARY, 21, 2);
